...
module module_WRF_HYDRO
    ...
    use NOAHMP_TABLES, only: ISWATER_TABLE   !=====||___WHQ___||=====! ***CHECK***
...
!  z1: array of soil layers of input variable.
      ...
      subroutine lsm_wrf_input(did,vegtyp,soltyp,ix,jx)
         ...
         where(soltyp == 14) VEGTYP = ISWATER_TABLE
         where(VEGTYP == ISWATER_TABLE ) soltyp = 14
         !
         !=====||___WHQ___||=====! ***CHECK***
...
!read parameter for LKSAT
      ...
      subroutine  checkSoil(did)
          ...
          where(rt_domain(did)%smc(:,:,1) <=0) RT_DOMAIN(did)%VEGTYP = ISWATER_TABLE
          where(rt_domain(did)%sh2ox(:,:,1) <=0) RT_DOMAIN(did)%VEGTYP = ISWATER_TABLE
          where(rt_domain(did)%smc(:,:,1) >=100) RT_DOMAIN(did)%VEGTYP = ISWATER_TABLE
          where(rt_domain(did)%sh2ox(:,:,1) >=100) RT_DOMAIN(did)%VEGTYP = ISWATER_TABLE
          !
          !=====||___WHQ___||=====! ***CHECK***
