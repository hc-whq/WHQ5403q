module config_base
  ...
  !=====||___WHQ___||=====!
  !
  type SedCNPmodel_OFFLINE_
     integer            :: SedCNP_option  !0: off (water only)
                                          !1: on (sediment only; need water outputs) - NOT AVAILABLE YET
                                          !2: on (CNP only; need water and sediment outputs)  - NOT AVAILABLE YET
                                          !3: on (sediment + CNP; need water outputs)

     integer            :: SedCNP_timestep
     integer            :: SedCNP_start_year
     integer            :: SedCNP_start_month
     integer            :: SedCNP_start_day
     integer            :: SedCNP_start_hour
     integer            :: SedCNP_start_min
  ...
  TYPE namelist_rt_
     ...
     character(len=256) :: CH_PNTSRC_file = ""   !=====||___WHQ___||=====!
     character(len=256) :: CH_ABSDIS_file = ""   !=====||___WHQ___||=====!
  ...
  type(SedCNPmodel_OFFLINE_), protected, save :: SedCNPmodel  !=====||___WHQ___||=====!
...
!!! add the following two dummy variables
    ...
    namelist /HYDRO_nlist/ NSOIL, ZSOIL8,&
         ...
         CH_PNTSRC_file, CH_ABSDIS_file, &      !=====||___WHQ___||=====!
...
! #endif
    ...
    nlst(did)%reservoir_rfc_forecasts_time_series_path = reservoir_rfc_forecasts_time_series_path
    nlst(did)%reservoir_rfc_forecasts_lookback_hours = reservoir_rfc_forecasts_lookback_hours
    nlst(did)%CH_PNTSRC_file = trim(CH_PNTSRC_file)   !=====||___WHQ___||=====!
    nlst(did)%CH_ABSDIS_file = trim(CH_ABSDIS_file)   !=====||___WHQ___||=====!
  ...
  subroutine init_noah_lsm_and_wrf_hydro()
    ...
    !=====||___WHQ___||=====!
    !
    integer            :: SedCNP_option = 0
    integer            :: SedCNP_start_year
    integer            :: SedCNP_start_month
    integer            :: SedCNP_start_day
    integer            :: SedCNP_start_hour
    integer            :: SedCNP_start_min
    integer            :: SedCNP_khour = -999
    integer            :: SedCNP_kday  = -999
    integer            :: SedCNP_timestep
    logical            :: SedCNP_compound_channel
    character(len=256) :: LDASOUT_dir, WHQOUT_dir, WHQIN_dir, &
    ...
    namelist / NOAHLSM_OFFLINE /    &
         SedCNP_option, &       !=====||___WHQ___||=====!
         config_paddy_file, &   !=====||___WHQ___||=====!
    ...
    !=====||___WHQ___||=====!
    !
    SedCNPmodel%SedCNP_start_year    = -999
    SedCNPmodel%SedCNP_start_month   = -999
    SedCNPmodel%SedCNP_start_day     = -999
    SedCNPmodel%SedCNP_start_hour    = -999
    SedCNPmodel%SedCNP_start_min     = -999
    SedCNPmodel%SedCNP_khour         = -999
    SedCNPmodel%SedCNP_kday          = -999
    SedCNPmodel%SedCNP_timestep      = -999
    !
    !=====||___WHQ___||=====!
...
!=====||___WHQ___||=====!
!
    if (SedCNP_option > 0) then
#ifndef NCEP_WCOSS
        open(40, file="whq.namelist", form="FORMATTED")
        read(40, NML=SedCNPmodel_OFFLINE, iostat=ierr)
#else
        open(14, form="FORMATTED")
        read(14, NML=SedCNPmodel_OFFLINE, iostat=ierr)
#endif
        if (ierr /= 0) then
           write(*,'(/," ***** ERROR: Problem reading namelist SedCNPmodel_OFFLINE",/)')
           call hydro_stop (" FATAL ERROR: Problem reading namelist SedCNPmodel_OFFLINE")
        endif
    end if
!
!=====||___WHQ___||=====!
...
#ifndef NCEP_WCOSS
    close(30)
    if (SedCNP_option > 0) close(40)      !=====||___WHQ___||=====!
#else
    close(11)
    if (SedCNP_option > 0) close(14)      !=====||___WHQ___||=====!
#endif
    ...
    SedCNPmodel%SedCNP_option      = SedCNP_option      !=====||___WHQ___||=====!
    ...
    !=====||___WHQ___||=====!
    !
    SedCNPmodel%SedCNP_start_year  = SedCNP_start_year
    SedCNPmodel%SedCNP_start_month = SedCNP_start_month
    SedCNPmodel%SedCNP_start_day   = SedCNP_start_day
    SedCNPmodel%SedCNP_start_hour  = SedCNP_start_hour
    SedCNPmodel%SedCNP_start_min   = SedCNP_start_min
    if (SedCNP_kday > 0) then
        if (SedCNP_khour > 0) then
            write(*, '("WARNING: Check Namelist: SedCNP_KHOUR and SedCNP_KDAY both defined, SedCNP_KHOUR will take precedence.")')
            SedCNP_kday = -999
        else
            write(*, '("WARNING: SedCNP_KDAY is deprecated and may be removed in a future version, please use SedCNP_KHOUR.")')
            SedCNP_khour = -999
        end if
    end if
    SedCNPmodel%SedCNP_kday             = SedCNP_kday
    ...
    SedCNPmodel%CiniSo_file             = trim(CiniSo_file)
    SedCNPmodel%NiniSo_file             = trim(NiniSo_file)
    SedCNPmodel%PiniSo_file             = trim(PiniSo_file)
    SedCNPmodel%CiniGw_file             = trim(CiniGw_file)
    SedCNPmodel%NiniGw_file             = trim(NiniGw_file)
    SedCNPmodel%PiniGw_file             = trim(PiniGw_file)
    SedCNPmodel%CiniCh_file             = trim(CiniCh_file)
    SedCNPmodel%NiniCh_file             = trim(NiniCh_file)
    SedCNPmodel%PiniCh_file             = trim(PiniCh_file)
    SedCNPmodel%CH_PNTSRC_file          = trim(CH_PNTSRC_file)
    SedCNPmodel%CH_ABSDIS_file          = trim(CH_ABSDIS_file)
    SedCNPmodel%CNPinputs_file          = trim(CNPinputs_file)
    SedCNPmodel%SOILPSF_file            = trim(SOILPSF_file)
