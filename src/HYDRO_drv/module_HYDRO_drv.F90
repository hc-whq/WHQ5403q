...
! restart every month automatically.
        ...
        if(rst_out .gt. 0) then
            ...
            if(nlst(did)%rst_bi_out .eq. 1) then
                ...
                ! Updated to use user-specified OUTDIR from namelist.hrldas
                call   RESTART_OUT_bi((trim(noah_lsm%outdir)//"/HYDRO_RST."//nlst(did)%olddate(1:16)   &
                    //"_DOMAIN"//trim(nlst(did)%hgrid)//"."//trim(str_tmp)),  did)
                !
                !=====||___WHQ___||=====!
            else
                ...
                ! Updated to use user-specified OUTDIR from namelist.hrldas
                call   RESTART_OUT_nc((trim(noah_lsm%outdir)//"/HYDRO_RST."//nlst(did)%olddate(1:16)   &
                    //"_DOMAIN"//trim(nlst(did)%hgrid)),  did)
                !
                !=====||___WHQ___||=====!
#ifdef MPP_LAND
...
!yw  endif
    ...
    subroutine HYDRO_time_adv(did)
        ...
        !=====||___WHQ___||=====! ***CHECK***
        !
        ! Broadcast the updated time to all processes to ensure synchronization
        call mpp_land_bcast_char(19, nlst(did)%olddate)
        !
        !=====||___WHQ___||=====! ***CHECK***
#endif
...
!-------------------------------------------
    subroutine driveChannelRouting(did)
        ...
        !
        integer :: itime
        integer :: year0, month0, day0, hour0, julian_day0, jh0
        integer :: year1, month1, day1, hour1, julian_day1, jh1

        read(nlst(did)%startdate(1:4), *) year0
        read(nlst(did)%startdate(6:7), *) month0
        read(nlst(did)%startdate(9:10), *) day0
        read(nlst(did)%startdate(12:13), *) hour0
        read(nlst(did)%olddate(1:4), *) year1
        read(nlst(did)%olddate(6:7), *) month1
        read(nlst(did)%olddate(9:10), *) day1
        read(nlst(did)%olddate(12:13), *) hour1
        ...
        itime = nint((jh1 - jh0) * 3600.0_r8 / real(nlst(did)%dt, r8)) + 1

        ! array slice for the current time-step
        RT_DOMAIN(did)%q_pnt(:) = RT_DOMAIN(did)%Qpnt(:,itime)
        RT_DOMAIN(did)%q_abs(:) = RT_DOMAIN(did)%Qabs(:,itime)
        RT_DOMAIN(did)%q_dis(:) = RT_DOMAIN(did)%Qdis(:,itime)
        !
        !=====||___WHQ___||=====! ***CHECK***
...
!!! for user defined Reach based Routing method.
                ...
                call drive_CHANNEL(did, RT_DOMAIN(did)%latval,RT_DOMAIN(did)%lonval, &
                    ...
                    !
                    , RT_DOMAIN(did)%q_pnt, &
                    RT_DOMAIN(did)%q_abs, RT_DOMAIN(did)%q_dis, itime &
                    !
                    !=====||___WHQ___||=====!
                    )
            endif
...
! need to set these after getChanDim since it allocates rt_domain vals to 0
        ...
        !
            ...
            ! drive_CHANNEL always receives gw2d%ho and gw2d%qgw_chanrt as actual
            ! arguments; allocate zero-filled dummies so the runtime allocation check
            ! passes even when the 2-D GW model is inactive.
            allocate(gw2d(did)%ho        (rt_domain(did)%ixrt, rt_domain(did)%jxrt))
            allocate(gw2d(did)%qgw_chanrt(rt_domain(did)%ixrt, rt_domain(did)%jxrt))
            gw2d(did)%ho         = 0.0
            gw2d(did)%qgw_chanrt = 0.0
        endif
        !
        !=====||___WHQ___||=====! ***CHECK***
...
!ADCHANGE: For consistency, mirror urban and param value checks used in table read
    ...
    !=====||___WHQ___||=====!
    !
    subroutine Read_PS_QInputs(did)
       ...
       integer            :: did
       integer            :: khour
       integer            :: ich, ich_pnt, reach_id
       integer            :: itime, pntsrc_ntime
       ...
       character(len=630), allocatable :: file_content(:)
       ...
       if ((khour < 0) .and. (noah_lsm%kday < 0)) then
          write(*, '("FATAL ERROR: In module_HYDRO_drv.F Read_PS_QInputs() - "// &
               "Namelist error: Either KHOUR or KDAY must be defined.")')
          call hydro_stop("FATAL ERROR: In Read_PS_QInputs() - KHOUR or KDAY must be defined.")
       else if (( khour < 0 ) .and. (noah_lsm%kday > 0)) then
          khour = noah_lsm%kday * 24
       else if ((khour > 0) .and. (noah_lsm%kday > 0)) then
          write(*, '("WARNING: In Read_PS_QInputs() - KHOUR and KDAY both defined. Using KHOUR.")')
       endif

       !pntsrc_ntime = khour*3600./nint(dts)
       if (nlst(did)%dt > 0.0) then
          pntsrc_ntime = nint(real(khour, r8) * 3600.0_r8 / real(nlst(did)%dt, r8))
       else
          call hydro_stop("FATAL ERROR: nlst(did)%dt is zero or negative in Read_PS_QInputs.")
          pntsrc_ntime = 1 ! Stop gap
       endif

       allocate(RT_DOMAIN(did)%Qpnt(RT_DOMAIN(did)%NLINKSL,pntsrc_ntime))
       ...
       !--- initialisation
       RT_DOMAIN(did)%Qpnt = 0.  !water inflow (m3 s-1)
       ich_pnt = 0
       str_date = ' '
       str_var = ' '
       ...
       DO line_idx = 1, num_lines
          ...
          if (str_line(1:5) == 'REACH') then
             ! A new reach is starting. Finalize the previous one if it exists.
             if (ich_pnt > 0) then
                do itime = max(1, t0), pntsrc_ntime
                   RT_DOMAIN(did)%Qpnt(ich_pnt,itime) = dQ0 / 3600.
                enddo
             endif

             read(str_line(7:), *, iostat=ierr) reach_id
             if (ierr /= 0) then
                call hydro_stop("Failed to read REACH ID (CH_PNTSRC)")
             endif

             ich_pnt = 0
             do ich = 1,RT_DOMAIN(did)%NLINKSL
                if(RT_DOMAIN(did)%linkid(ich)==reach_id) then
                   ich_pnt = ich
                   exit
                endif
             enddo
             ...
             read(nlst(did)%startdate(9:10), *) day0
             read(nlst(did)%startdate(12:13), *) hour0
             read(nlst(did)%startdate(15:16), *) minute0
             t0 = 1
             dQ0 = 0.0

          elseif (str_line(1:3) == 'END') then
             ! End of all data. Finalize the last reach block
             if (ich_pnt > 0) then
                do itime = max(1, t0), pntsrc_ntime
                   RT_DOMAIN(did)%Qpnt(ich_pnt,itime) = dQ0 / 3600.
                enddo
             ...
             ! This is potentially a data line.
             if (len_trim(str_line) > 0 .and. &
                 (ichar(str_line(1:1)) >= ichar('0') .and. ichar(str_line(1:1)) <= ichar('9'))) then
                ! This is a data line. Only the owning process stores the data.
                ...
                write(*,*) 'WARNING: Skipping unrecognized line in CH_PNTSRC file: ', trim(str_line)
                cycle
             endif

             str_date = adjustl(str_date)
             ...
             !Add bounds check before calculating t1
             if (t0 <= pntsrc_ntime) then
                if (nlst(did)%dt > 0.0) then
                   t1 = t0 + nint((jh1 - jh0) * 3600.0_r8 / real(nlst(did)%dt, r8))
                else
                   call hydro_stop("FATAL ERROR: nlst(did)%dt is zero or negative in Read_PS_QInputs.")
                   t1 = t0 + 1 ! Stop gap
                endif
             ...
             if (ich_pnt > 0) then
                do itime = max(1, t0), t1 - 1
                   if (itime > pntsrc_ntime) exit
                   RT_DOMAIN(did)%Qpnt(ich_pnt,itime) = dQ0 / 3600.
                enddo
             endif
       ...
       ! In case the file does not have an 'END' tag, finalize the last reach.
       if (ich_pnt > 0) then
          do itime = max(1, t0), pntsrc_ntime
             RT_DOMAIN(did)%Qpnt(ich_pnt,itime) = dQ0 / 3600.
          enddo
    ...
    end subroutine Read_PS_QInputs
    !
    !=====||___WHQ___||=====!

    !=====||___WHQ___||=====!
    !
    subroutine Read_ABSDIS_QInputs(did)

       use config_base, only: nlst, noah_lsm, SedCNPmodel
       use module_RT_data, only: RT_DOMAIN
       ...
       integer            :: did
       integer            :: khour
       integer            :: ich, ich_absdis, reach_id
       integer            :: itime, absdis_ntime
       integer            :: year0, month0, day0, hour0, minute0, julian_day0, jh0, t0
       integer            :: year1, month1, day1, hour1, minute1, julian_day1, jh1, t1
       integer            :: ierr
       real(8)            :: dQ0, dQ1
       ...
       if ((khour < 0) .and. (noah_lsm%kday < 0)) then
          write(*, '("FATAL ERROR: In module_HYDRO_drv.F Read_ABSDIS_QInputs() - "// &
               "Namelist error: Either KHOUR or KDAY must be defined.")')
          call hydro_stop("FATAL ERROR: In Read_ABSDIS_QInputs() - KHOUR or KDAY must be defined.")
       else if (( khour < 0 ) .and. (noah_lsm%kday > 0)) then
          khour = noah_lsm%kday * 24
       else if ((khour > 0) .and. (noah_lsm%kday > 0)) then
          write(*, '("WARNING: In Read_ABSDIS_QInputs() - KHOUR and KDAY both defined. Using KHOUR.")')
       endif

       !absdis_ntime = khour*3600./nint(dts)
       if (nlst(did)%dt > 0.0) then
          absdis_ntime = nint(real(khour, r8) * 3600.0_r8 / real(nlst(did)%dt, r8))
       else
          call hydro_stop("FATAL ERROR: nlst(did)%dt is zero or negative in Read_ABSDIS_QInputs.")
          absdis_ntime = 1 ! Stop gap
       endif

       allocate(RT_DOMAIN(did)%Qabs(RT_DOMAIN(did)%NLINKSL,absdis_ntime))
       allocate(RT_DOMAIN(did)%Qdis(RT_DOMAIN(did)%NLINKSL,absdis_ntime))
       ...
       !--- initialisation
       RT_DOMAIN(did)%Qabs = 0.
       RT_DOMAIN(did)%Qdis = 0.
       ich_absdis = 0
       str_date = ' '
       str_var = ' '
       ...
       DO line_idx = 1, num_lines
          ...
          if (str_line(1:5) == 'REACH') then
             ...
             if (ich_absdis > 0) then
                if (dQ0 <= 0.) then  ! Abstraction
                   do itime = max(1, t0), absdis_ntime
                      RT_DOMAIN(did)%Qabs(ich_absdis,itime) = -1.0 * dQ0 / 3600.
                   enddo
                else ! Discharge
                   do itime = max(1, t0), absdis_ntime
                      RT_DOMAIN(did)%Qdis(ich_absdis,itime) = RT_DOMAIN(did)%Qdis(ich_absdis,itime) + dQ0 / 3600.
                   enddo
                endif
             endif
          ...
          elseif (str_line(1:3) == 'END') then
             ...
             if (len_trim(str_line) > 0 .and. &
                ...
                read(str_line, *, iostat=ierr) str_date, str_var
                if (ierr > 0) then
                   write(*,*) 'Format error in CH_ABSDIS data file: ', trim(str_line)
                   call hydro_stop("Failed to read input data (CH_ABSDIS)")
                endif
             else
                write(*,*) 'WARNING: Skipping unrecognized line in CH_ABSDIS file: ', trim(str_line)
                cycle
             endif
    ...
    end subroutine Read_ABSDIS_QInputs
    !
    !=====||___WHQ___||=====!
