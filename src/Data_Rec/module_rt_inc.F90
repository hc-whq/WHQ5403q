...
!=====||___WHQ___||=====!
!
!!! channel inflow and outflow - point source, abstraction and discharge
  REAL, allocatable, DIMENSION(:)      :: q_pnt     !inflow from point source (m^3/s)
  REAL, allocatable, DIMENSION(:,:)    :: Qpnt      !inflow from point source (m^3/s)
  REAL, allocatable, DIMENSION(:)      :: q_abs      !outflow by abstraction (m^3/s)
  REAL, allocatable, DIMENSION(:,:)    :: Qabs       !outflow by abstraction (m^3/s)
  REAL, allocatable, DIMENSION(:)      :: q_dis      !inflow by discharge (m^3/s)
  REAL, allocatable, DIMENSION(:,:)    :: Qdis       !inflow by discharge (m^3/s)
!
!=====||___WHQ___||=====!
...
!-- lake params
  ...
  !=====||___WHQ___||=====!
  !
  REAL, allocatable, DIMENSION(:) :: slc_elev          ! sluice gate spillway elevation (m)
  REAL, allocatable, DIMENSION(:) :: slc_length        ! sluice gate length (m)
  REAL, allocatable, DIMENSION(:) :: slc_coeff         ! sluice gate discharge coefficient (-)
  REAL, allocatable, DIMENSION(:) :: slc_open          ! sluice gate opening height (m)
  REAL, allocatable, DIMENSION(:) :: flp_elev          ! flap gate overflow elevation, when closed (m)
  REAL, allocatable, DIMENSION(:) :: flp_length        ! flap gate length (m)
  REAL, allocatable, DIMENSION(:) :: flp_coeff         ! flap gate discharge coefficient (-)
  REAL, allocatable, DIMENSION(:) :: flp_open          ! flap gate opening height (m)
  REAL, allocatable, DIMENSION(:) :: rad_elev          ! radial gate spillway elevation (m)
  REAL, allocatable, DIMENSION(:) :: rad_length        ! radial gate length (m)
  REAL, allocatable, DIMENSION(:) :: rad_coeff         ! radial gate discharge coefficient (-)
  REAL, allocatable, DIMENSION(:) :: rad_open          ! radial gate opening height (m)
  REAL, allocatable, DIMENSION(:) :: rad_htrn          ! radial gate trunnion height, from crest to pivot axis (m)
  REAL, allocatable, DIMENSION(:) :: rad_tau           ! radial gate exponent for trunnion height (-)
  REAL, allocatable, DIMENSION(:) :: rad_beta          ! radial gate exponent for opening height (-)
  REAL, allocatable, DIMENSION(:) :: rad_eta           ! radial gate exponent for upstream water head (-)
  !
  !=====||___WHQ___||=====!
