...
! follwoing is modified by Wei Yu 03/24/2017
     ...
     if(UDMP_OPT .eq. 0 .and. channel_option .eq. 3) then  !from WH v5.1.1
     !
     !=====||___WHQ___||=====!
        NLAKES = 0
        do j=1,jxrt
...
!     !- flow definition, var
        ...
        !if(UDMP_OPT .eq. 1) then  !=====||___WHQ___||=====! !commented out
...
!=====||___WHQ___||=====!
!
real, intent(inout), dimension(:)    :: slc_elev         ! sluice gate spillway elevation (m)
real, intent(inout), dimension(:)    :: slc_length       ! sluice gate length (m)
real, intent(inout), dimension(:)    :: slc_coeff        ! sluice gate discharge coefficient (-)
real, intent(inout), dimension(:)    :: slc_open         ! sluice gate opening height (m)
real, intent(inout), dimension(:)    :: flp_elev         ! flap gate overflow elevation, when closed (m)
real, intent(inout), dimension(:)    :: flp_length       ! flap gate length (m)
real, intent(inout), dimension(:)    :: flp_coeff        ! flap gate discharge coefficient (-)
real, intent(inout), dimension(:)    :: flp_open         ! flap gate opening height (m)
real, intent(inout), dimension(:)    :: rad_elev         ! radial gate spillway elevation (m)
real, intent(inout), dimension(:)    :: rad_length       ! radial gate length (m)
real, intent(inout), dimension(:)    :: rad_coeff        ! radial gate discharge coefficient (-)
real, intent(inout), dimension(:)    :: rad_open         ! radial gate opening height (m)
real, intent(inout), dimension(:)    :: rad_htrn         ! radial gate trunnion height, from crest to pivot axis (m)
real, intent(inout), dimension(:)    :: rad_tau          ! radial gate exponent for trunnion height (-)
real, intent(inout), dimension(:)    :: rad_beta         ! radial gate exponent for opening height (-)
real, intent(inout), dimension(:)    :: rad_eta          ! radial gate exponent for upstream water head (-)
!
!=====||___WHQ___||=====!
...
5101      continue
   ...
   if (NLAKES > 0) then
      ...
      !=====||___WHQ___||=====!
      !
      call mpp_land_bcast_real(NLAKES, slc_elev)
      call mpp_land_bcast_real(NLAKES, slc_length)
      call mpp_land_bcast_real(NLAKES, slc_coeff)
      call mpp_land_bcast_real(NLAKES, slc_open)
      call mpp_land_bcast_real(NLAKES, flp_elev)
      call mpp_land_bcast_real(NLAKES, flp_length)
      call mpp_land_bcast_real(NLAKES, flp_coeff)
      call mpp_land_bcast_real(NLAKES, flp_open)
      call mpp_land_bcast_real(NLAKES, rad_elev)
      call mpp_land_bcast_real(NLAKES, rad_length)
      call mpp_land_bcast_real(NLAKES, rad_coeff)
      call mpp_land_bcast_real(NLAKES, rad_open)
      call mpp_land_bcast_real(NLAKES, rad_htrn)
      call mpp_land_bcast_real(NLAKES, rad_tau)
      call mpp_land_bcast_real(NLAKES, rad_beta)
      call mpp_land_bcast_real(NLAKES, rad_eta)
      !
      !=====||___WHQ___||=====!
...
!NLAKES
        ...
        !=====||___WHQ___||=====!
        !
        real, intent(out), dimension(:)    :: slc_elev         ! sluice gate spillway elevation (m)
        real, intent(out), dimension(:)    :: slc_length       ! sluice gate length (m)
        real, intent(out), dimension(:)    :: slc_coeff        ! sluice gate discharge coefficient (-)
        real, intent(out), dimension(:)    :: slc_open         ! sluice gate opening height (m)
        real, intent(out), dimension(:)    :: flp_elev         ! flap gate overflow elevation, when closed (m)
        real, intent(out), dimension(:)    :: flp_length       ! flap gate length (m)
        real, intent(out), dimension(:)    :: flp_coeff        ! flap gate discharge coefficient (-)
        real, intent(out), dimension(:)    :: flp_open         ! flap gate opening height (m)
        real, intent(out), dimension(:)    :: rad_elev         ! radial gate spillway elevation (m)
        real, intent(out), dimension(:)    :: rad_length       ! radial gate length (m)
        real, intent(out), dimension(:)    :: rad_coeff        ! radial gate discharge coefficient (-)
        real, intent(out), dimension(:)    :: rad_open         ! radial gate opening height (m)
        real, intent(out), dimension(:)    :: rad_htrn         ! radial gate trunnion height, from crest to pivot axis (m)
        real, intent(out), dimension(:)    :: rad_tau          ! radial gate exponent for trunnion height (-)
        real, intent(out), dimension(:)    :: rad_beta         ! radial gate exponent for opening height (-)
        real, intent(out), dimension(:)    :: rad_eta          ! radial gate exponent for upstream water head (-)
        !
        !=====||___WHQ___||=====!
!end NLAKES
...
!=====||___WHQ___||=====!
!
real, intent(inout), dimension(NLAKES)    :: slc_elev         ! sluice gate spillway elevation (m)
real, intent(inout), dimension(NLAKES)    :: slc_length       ! sluice gate length (m)
real, intent(inout), dimension(NLAKES)    :: slc_coeff        ! sluice gate discharge coefficient (-)
real, intent(inout), dimension(NLAKES)    :: slc_open         ! sluice gate opening height (m)
real, intent(inout), dimension(NLAKES)    :: flp_elev         ! flap gate overflow elevation, when closed (m)
real, intent(inout), dimension(NLAKES)    :: flp_length       ! flap gate length (m)
real, intent(inout), dimension(NLAKES)    :: flp_coeff        ! flap gate discharge coefficient (-)
real, intent(inout), dimension(NLAKES)    :: flp_open         ! flap gate opening height (m)
real, intent(inout), dimension(NLAKES)    :: rad_elev         ! radial gate spillway elevation (m)
real, intent(inout), dimension(NLAKES)    :: rad_length       ! radial gate length (m)
real, intent(inout), dimension(NLAKES)    :: rad_coeff        ! radial gate discharge coefficient (-)
real, intent(inout), dimension(NLAKES)    :: rad_open         ! radial gate opening height (m)
real, intent(inout), dimension(NLAKES)    :: rad_htrn         ! radial gate trunnion height, from crest to pivot axis (m)
real, intent(inout), dimension(NLAKES)    :: rad_tau          ! radial gate exponent for trunnion height (-)
real, intent(inout), dimension(NLAKES)    :: rad_beta         ! radial gate exponent for opening height (-)
real, intent(inout), dimension(NLAKES)    :: rad_eta          ! radial gate exponent for upstream water head (-)
!
!=====||___WHQ___||=====!
...
subroutine read_route_lake_netcdf(route_lake_file,                         &
    ...
    !=====||___WHQ___||=====!
    !
    !call get_1d_netcdf_real(ncid, 'slc_elev',    slc_elev,     'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'slc_length',  slc_length,   'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'slc_coeff',   slc_coeff,    'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'slc_open',    slc_open,     'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'flp_elev',    flp_elev,     'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'flp_length',  flp_length,   'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'flp_coeff',   flp_coeff,    'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'flp_open',    flp_open,     'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'rad_elev',    rad_elev,     'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'rad_length',  rad_length,   'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'rad_coeff',   rad_coeff,    'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'rad_open',    rad_open,     'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'rad_htrn',    rad_htrn,     'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'rad_tau',     rad_tau,      'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'rad_beta',    rad_beta,     'read_route_lake_netcdf', .TRUE.)
    !call get_1d_netcdf_real(ncid, 'rad_eta',     rad_eta,      'read_route_lake_netcdf', .TRUE.)
    !
    !=====||___WHQ___||=====!
...
!#endif
	...
	if ( (channel_option .ne. 3) .and. (io_config_outputs .ge. 0) ) then
      ...
      !=====||___WHQ___||=====!
      !
      !-----water inflow from point source
      !if ( (channel_option .ne. 3) ) then
         iret = nf90_def_var(ncid, "Qpnt", NF90_FLOAT, (/stationdim/), varid)
         iret = nf90_put_att(ncid, varid, 'units', 'meter^3 / sec')
         iret = nf90_put_att(ncid, varid, 'long_name', 'Water inflow from point sources')
      !endif

      !-----water outflow by abstraction
      !if ( (channel_option .ne. 3) ) then
         iret = nf90_def_var(ncid, "Qabs", NF90_FLOAT, (/stationdim/), varid)
         iret = nf90_put_att(ncid, varid, 'units', 'meter^3 / sec')
         iret = nf90_put_att(ncid, varid, 'long_name', 'Water outflow by abstraction')
      !endif

      !-----water inflow by discharge
      !if ( (channel_option .ne. 3) ) then
         iret = nf90_def_var(ncid, "Qdis", NF90_FLOAT, (/stationdim/), varid)
         iret = nf90_put_att(ncid, varid, 'units', 'meter^3 / sec')
         iret = nf90_put_att(ncid, varid, 'long_name', 'Water inflow by discharge')
      !endif
      !
      !=====||___WHQ___||=====!
...
!#endif
	...
	if ( (channel_option .ne. 3) .and. (io_config_outputs .ge. 0) ) then
       ...
       if(UDMP_OPT .eq. 1 .and. nlst(did)%output_channelBucket_influx .ne. 0) then
         ...
         !
         !if ( (channel_option .ne. 3) ) then
            iret = nf90_inq_varid(ncid,"Qpnt", varid)
            iret = nf90_put_var(ncid, varid, RT_DOMAIN(did)%q_pnt, (/1/), (/nstations/))
            iret = nf90_inq_varid(ncid,"Qabs", varid)
            iret = nf90_put_var(ncid, varid, RT_DOMAIN(did)%q_abs, (/1/), (/nstations/))
            iret = nf90_inq_varid(ncid,"Qdis", varid)
            iret = nf90_put_var(ncid, varid, RT_DOMAIN(did)%q_dis, (/1/), (/nstations/))
         !endif
         !
         !=====||___WHQ___||=====!
...
!!! Define variables
      ...
      !iret = nf90_put_att(ncid, varid, 'units', 'mm')
      iret = nf90_put_att(ncid, varid, 'units', 'm')  !=====||___WHQ___||=====! (original variable z_gwsubbas is in 'm')
