...
! ------------------------------------------------
     Subroutine drive_CHANNEL(did, latval,lonval,KT, IXRT,JXRT, SUBRTSWCRT, &
       ...
       real , dimension(ixrt,jxrt):: latval,lonval
        ...
        !=====||___WHQ___||=====!
        !
        real, intent(inout), dimension(NLINKSL)   :: q_pnt
        real, intent(inout), dimension(NLINKSL)   :: q_abs
        real, intent(inout), dimension(NLINKSL)   :: q_dis
...
!DJG GW-chan coupling variables...
        ...
        integer, intent(in), dimension(:)     :: lake_lookup  !-- inverse lake index for k->lake mapping !=====||___WHQ___||=====!

!-- Local Variables
        INTEGER                     :: i,j,k,t,m,jj,kk,KRT,node,l_idx, lakeid  !=====||___WHQ___||=====!
        INTEGER                     :: DT_STEPS               !-- number of timestep in routing
        integer                     :: ich_dis   !reach-index where abstraction water to be discharged   !=====||___WHQ___||=====!
        integer                     :: itime_dis  !itime when abstraction water to be discharged         !=====||___WHQ___||=====!
        ...
        real*8                      :: Tw_stream, Wstorage    !=====||___WHQ___||=====!
...
!      print *, "DRIVE_channel, RESHT", RESHT
   ...
   IF(channel_option .ne. 3) then   !--muskingum methods ROUTE ON DT timestep, not DTRT!!
         ...
         !
         do k = 1,NLINKSL
            q_abs(k) = min(q_abs(k), 0.1 * QLINK(k,1))             !m3 s-1
            q_dis(k) = RT_DOMAIN(did)%Qdis(k,itime)                !m3 s-1
         enddo

         !update the arrays
         do k = 1, NLINKSL
            RT_DOMAIN(did)%Qabs(k,itime) = q_abs(k)
            RT_DOMAIN(did)%Qdis(k,itime) = q_dis(k)
         enddo
         !
         !=====||___WHQ___||=====!
...
!yw                  endif
               ...
               do m = 1, NLINKSL
                ...
                elseif (channel_option .eq. 1) then  !muskingum routing
                   ...
                   QLateral(k) = QLateral(k) + (q_pnt(k) + q_dis(k) - q_abs(k)) / nsteps   !=====||___WHQ___||=====!

                  !=====||___WHQ___||=====!
                  !
                  if(isnan(HLINK(k))) then
                     HLINK(k) = 0.01
                  endif
                  !
                  !=====||___WHQ___||=====!
