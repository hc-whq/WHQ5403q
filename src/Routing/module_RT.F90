...
!DJG Allocate routing and disaggregation arrays
  ...
  !=====||___WHQ___||=====!
  !
  allocate(subbasinID(IXRT,JXRT))
           subbasinID = -9999
  allocate(q_sogw(IXRT,JXRT))
           q_sogw = -9999
  allocate(q_intf(IXRT,JXRT))
           q_intf = -9999
  !
  !=====||___WHQ___||=====!
...
!tmp  endif
     ...
     if(NLAKES .gt. 0) then
        ...
        !=====||___WHQ___||=====!
        !
        allocate( rt_domain(did)%slc_elev(NLAKES) )
        allocate( rt_domain(did)%slc_length(NLAKES) )
        allocate( rt_domain(did)%slc_coeff(NLAKES) )
        allocate( rt_domain(did)%slc_open(NLAKES) )
        allocate( rt_domain(did)%flp_elev(NLAKES) )
        allocate( rt_domain(did)%flp_length(NLAKES) )
        allocate( rt_domain(did)%flp_coeff(NLAKES) )
        allocate( rt_domain(did)%flp_open(NLAKES) )
        allocate( rt_domain(did)%rad_elev(NLAKES) )
        allocate( rt_domain(did)%rad_length(NLAKES) )
        allocate( rt_domain(did)%rad_coeff(NLAKES) )
        allocate( rt_domain(did)%rad_open(NLAKES) )
        allocate( rt_domain(did)%rad_htrn(NLAKES) )
        allocate( rt_domain(did)%rad_tau(NLAKES) )
        allocate( rt_domain(did)%rad_beta(NLAKES) )
        allocate( rt_domain(did)%rad_eta(NLAKES) )
        !
        !=====||___WHQ___||=====!
         ...
         !=====||___WHQ___||=====!
         !
         rt_domain(did)%slc_elev = 0.0
         rt_domain(did)%slc_length = 0.0
         rt_domain(did)%slc_coeff = 0.0
         rt_domain(did)%slc_open = 0.0
         rt_domain(did)%flp_elev = 0.0
         rt_domain(did)%flp_length = 0.0
         rt_domain(did)%flp_coeff = 0.0
         rt_domain(did)%flp_open = 0.0
         rt_domain(did)%rad_elev = 0.0
         rt_domain(did)%rad_length = 0.0
         rt_domain(did)%rad_coeff = 0.0
         rt_domain(did)%rad_open = 0.0
         rt_domain(did)%rad_htrn = 0.0
         rt_domain(did)%rad_tau = 0.0
         rt_domain(did)%rad_beta = 0.0
         rt_domain(did)%rad_eta = 0.0
         !
         !=====||___WHQ___||=====!
...
!    allocate( rt_domain(did)%ORIFICEE(nsizes) )
	...
	rt_domain(did)%velocity  = zeroFlt
         ...
         !=====||___WHQ___||=====!
         !
         allocate( rt_domain(did)%q_pnt(NLINKSL) )
         allocate( rt_domain(did)%q_abs(NLINKSL) )
         allocate( rt_domain(did)%q_dis(NLINKSL) )
         rt_domain(did)%q_pnt  = zeroFlt
         rt_domain(did)%q_abs  = zeroFlt
         rt_domain(did)%q_dis  = zeroFlt
         !
         !=====||___WHQ___||=====!
  ...
  if (nlst(did)%SUBRTSWCRT  .eq.1 .or. &
   ...
   !yw CALL READ_ROUTING_old(rt_domain(did)%IXRT,rt_domain(did)%JXRT,rt_domain(did)%ELRT,rt_domain(did)%overland%streams_and_lakes%ch_netrt, &
        ...
        if (nlst(did)%CHANRTSWCRT.eq.1 .or. nlst(did)%CHANRTSWCRT .eq. 2) then  !IF/then for channel routing
           ...
           call MPP_READ_CHROUTING_new(    &
             ...
             !=====||___WHQ___||=====!
             !
             ,rt_domain(did)%slc_elev, rt_domain(did)%slc_length, rt_domain(did)%slc_coeff, rt_domain(did)%slc_open, &
             rt_domain(did)%flp_elev, rt_domain(did)%flp_length, rt_domain(did)%flp_coeff, rt_domain(did)%flp_open, &
             rt_domain(did)%rad_elev, rt_domain(did)%rad_length, rt_domain(did)%rad_coeff, rt_domain(did)%rad_open, &
             rt_domain(did)%rad_htrn, rt_domain(did)%rad_tau, rt_domain(did)%rad_beta, rt_domain(did)%rad_eta       &
             !
             !=====||___WHQ___||=====!
       ...
       select type (reservoir => rt_domain(did)%reservoirs(lake_index)%ptr)
          ...
          type is (levelpool)
               call reservoir%init(                               &
                     ...
                     !=====||___WHQ___||=====!
                     !
                     rt_domain(did)%slc_elev(lake_index), &        ! sluice gate spillway elevation (m)
                     rt_domain(did)%slc_length(lake_index), &      ! sluice gate length (m)
                     rt_domain(did)%slc_coeff(lake_index), &       ! sluice gate discharge coefficient (-)
                     rt_domain(did)%slc_open(lake_index), &         ! sluice gate opening height (m)
                     rt_domain(did)%flp_elev(lake_index), &         ! flap gate overflow elevation, when closed (m)
                     rt_domain(did)%flp_length(lake_index), &       ! flap gate length (m)
                     rt_domain(did)%flp_coeff(lake_index), &        ! flap gate discharge coefficient (-)
                     rt_domain(did)%flp_open(lake_index), &         ! flap gate opening height (m)
                     rt_domain(did)%rad_elev(lake_index), &         ! radial gate spillway elevation (m)
                     rt_domain(did)%rad_length(lake_index), &       ! radial gate length (m)
                     rt_domain(did)%rad_coeff(lake_index), &        ! radial gate discharge coefficient (-)
                     rt_domain(did)%rad_open(lake_index), &         ! radial gate opening height (m)
                     rt_domain(did)%rad_htrn(lake_index), &         ! radial gate trunnion height, from crest to pivot axis (m)
                     rt_domain(did)%rad_tau(lake_index), &          ! radial gate exponent for trunnion height (-)
                     rt_domain(did)%rad_beta(lake_index), &         ! radial gate exponent for opening height (-)
                     rt_domain(did)%rad_eta(lake_index) &           ! radial gate exponent for upstream water head (-)
                     !
                     !=====||___WHQ___||=====!
   ...
   !ADCHANGE: Add lake reach output
     ...
     if ( (nlst(did)%CHANRTSWCRT .eq. 1) .and. &
...
#ifdef HYDRO_D
           !write(6,*) "LNLINKSL, NLINKS, GNLINKS =",rt_domain(did)%LNLINKSL,rt_domain(did)%NLINKSL,rt_domain(did)%GNLINKSL   !original
           write(6,*) "LNLINKSL, NLINKS, GNLINKSL =", &
                  rt_domain(did)%LNLINKSL,rt_domain(did)%NLINKS,rt_domain(did)%GNLINKSL  !=====||___WHQ___||=====!  typo fixed
           call flush(6)
#endif
        ...
        !-------------------------------------------
        new_start_i = 1; new_start_j = 1      !=====||___WHQ___||=====! ***CHECK***
        new_end_i = rt_domain(did)%ixrt; new_end_j = rt_domain(did)%jxrt

        !if(left_id .ge. 0) new_start_i = 1  !original
        if(left_id .ge. 0) new_start_i = 2  !=====||___WHQ___||=====! ***CHECK***
...
!-----------------------------------------------------------------------
  ...
  if(nlst(did)%channel_only       .eq. 0 .and. &
     ...
     !=====||___WHQ___||=====! ***CHECK*** commented out redundent code
     !
   ...
   !=====||___WHQ___||=====!
   !
   if(allocated(rt_domain(did)%slc_elev)) deallocate(rt_domain(did)%slc_elev)
   if(allocated(rt_domain(did)%slc_length)) deallocate(rt_domain(did)%slc_length)
   if(allocated(rt_domain(did)%slc_coeff)) deallocate(rt_domain(did)%slc_coeff)
   if(allocated(rt_domain(did)%slc_open)) deallocate(rt_domain(did)%slc_open)
   if(allocated(rt_domain(did)%flp_elev)) deallocate(rt_domain(did)%flp_elev)
   if(allocated(rt_domain(did)%flp_length)) deallocate(rt_domain(did)%flp_length)
   if(allocated(rt_domain(did)%flp_coeff)) deallocate(rt_domain(did)%flp_coeff)
   if(allocated(rt_domain(did)%flp_open)) deallocate(rt_domain(did)%flp_open)
   if(allocated(rt_domain(did)%rad_elev)) deallocate(rt_domain(did)%rad_elev)
   if(allocated(rt_domain(did)%rad_length)) deallocate(rt_domain(did)%rad_length)
   if(allocated(rt_domain(did)%rad_coeff)) deallocate(rt_domain(did)%rad_coeff)
   if(allocated(rt_domain(did)%rad_open)) deallocate(rt_domain(did)%rad_open)
   if(allocated(rt_domain(did)%rad_htrn)) deallocate(rt_domain(did)%rad_htrn)
   if(allocated(rt_domain(did)%rad_tau)) deallocate(rt_domain(did)%rad_tau)
   if(allocated(rt_domain(did)%rad_beta)) deallocate(rt_domain(did)%rad_beta)
   if(allocated(rt_domain(did)%rad_eta)) deallocate(rt_domain(did)%rad_eta)
   !
   !=====||___WHQ___||=====!
